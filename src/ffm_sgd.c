...
void ffm_fit_sgd(ffm_coef * coef, cs *A, ffm_vector *y, ffm_param *param){
    ...
    int n_samples = A->n;
    int k; if (coef->V) k = coef->V->size0; else k = 0;
    if (!param->warm_start)
    ...
    for (int i=0; i<param->n_iter; i++){
        ...
        if (param->TASK == TASK_REGRESSION)
            ...
            y_err = (ffm_sigmoid(
                ffm_predict_sample(coef, A, sample_row) * y_true) -1 ) * y_true;
        coef->w_0 = coef->w_0 - step_size * y_err;
        // update w
        int p, *Ap, *Ai ; double *Ax ;
        Ap = A->p ; Ai = A->i ; Ax = A->x ;
        for (p = Ap [sample_row] ; p < Ap [sample_row+1] ; p++){
            double theta_w = ffm_vector_get(coef->w, Ai [p]);
            ffm_vector_set(coef->w, Ai [p], theta_w -
                 step_size * (y_err * Ax [p] + theta_w * coef->lambda_w));
        }
        if (k > 0)
            // update V
            for (int f=0; f<k; f++){
...
void ffm_fit_sgd_bpr(ffm_coef * coef, cs *A, ffm_matrix *pairs, ffm_param param){
    ...
    for (int i=0; i<param.n_iter; i++){
        ...
        // predict y(x)
        double pairs_err = -1 + ffm_sigmoid(ffm_predict_sample(coef, A, sample_row_p)-
        ...
        while (p_n < Ap[sample_row_n+1] || p_p < Ap[sample_row_p+1]){
            ...
            ffm_vector_set(coef->w, i_to_update, theta_w -
                step_size * (pairs_err * grad + theta_w * coef->lambda_w));
        }
        if (k > 0)
            // update V
            for (int f=0; f<k; f++){
                ...
                update_second_order_bpr(A, coef->V, cache_n, cache_p, pairs_err,
                        step_size, lambda_V_f, sample_row_p, sample_row_n, f);
            }
        }
}


void update_second_order_bpr(cs *A, ffm_matrix *V, double cache_n, double cache_p,
        double pairs_err, double step_size, double lambda_V,
        int sample_row_p, int sample_row_n, int f){

    int p_n, p_p, *Ap, *Ai, i_to_update ; double *Ax;
    Ap = A->p ; Ai = A->i ; Ax = A->x ;
    p_n = Ap[sample_row_n];
    p_p = Ap[sample_row_p];
    // go over both rows simultaneously,
        ...
        double stepsize)
{
    ...
    int k; if (coef_t0->V) k = coef_t0->V->size0; else k = 0;
    ...
    for (int i=0; i< n_features; i++)
        ffm_vector_set(grad->w, i, -
                (ffm_vector_get(coef_t1->w, i) / stepsize -
