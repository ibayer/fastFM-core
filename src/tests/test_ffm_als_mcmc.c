...
void test_sparse_update_v_ij(TestFixture_T* pFix, gconstpointer pg){
    ...
    ffm_vector_scale(err, -1); // account for sign change in error due to refactoring
    // nominator 658.82
    // denominator 1286.2500
    // 0.51220602526724979
    double sum_denominator, sum_nominator;
    sum_nominator = sum_denominator = 0;
    sparse_v_lf_frac(&sum_denominator, &sum_nominator, pFix->X, column_index, err, cache,
            a_theta, old_v_lf);
...
void test_sparse_als_zero_order_only(TestFixture_T* pFix, gconstpointer pg){
    ...
    int k = 0;
    ffm_param param = {.n_iter=1, .warm_start=true, .ignore_w=true,
        .init_sigma=0.1, .SOLVER=SOLVER_ALS, .TASK=TASK_REGRESSION};
...
void test_sparse_als_first_order_only(TestFixture_T* pFix, gconstpointer pg){
    ...
    int n_features = pFix->X->n;
    int k = 0;
    ffm_param param = {.n_iter=1, .warm_start=true, .ignore_w_0=true,
        .init_sigma=0.1, .SOLVER=SOLVER_ALS, .TASK=TASK_REGRESSION};
...
void test_sparse_als_second_order_only(TestFixture_T* pFix, gconstpointer pg){
    ...
    int k = 1;
    ffm_param param = {.n_iter=1, .warm_start=true, .ignore_w_0=true,
        .ignore_w=true, .init_sigma=0.1, .SOLVER=SOLVER_ALS, .TASK=TASK_REGRESSION};
    ...
    g_assert_cmpfloat(fabs(0.79866412 - ffm_matrix_get(coef->V, 0, 0)),
            < , 1e-8);
    g_assert_cmpfloat(fabs(400 - ffm_matrix_get(coef->V, 0, 1)),
            < , 1e-8);
...
void test_sparse_als_all_interactions(TestFixture_T* pFix, gconstpointer pg){
    ...
    int n_features = pFix->X->n;
    int k = 1;
    ffm_param param = {.n_iter=1, .warm_start=true, .ignore_w_0=false,
        .ignore_w=false, .init_sigma=0.1, .SOLVER=SOLVER_ALS, .TASK=TASK_REGRESSION};
    ...
    g_assert_cmpfloat(fabs(-191459.71428571 - ffm_vector_get(coef->w,0)), < , 1e-6);
    ...
    g_assert_cmpfloat(fabs(-322970.68313639 - ffm_vector_get(coef->w,0)), < , 1e-6);
    g_assert_cmpfloat(fabs(51927.60978978 - ffm_vector_get(coef->w,1)), < , 1e-6);
    g_assert_cmpfloat(fabs(94.76612018 - ffm_matrix_get(coef->V, 0, 0)),
            < , 1e-6);
    g_assert_cmpfloat(fabs(400 - ffm_matrix_get(coef->V, 0, 1)),
            < , 1e-6);
...
void test_sparse_als_first_order_interactions(TestFixture_T* pFix, gconstpointer pg){
    ffm_vector * y_pred = ffm_vector_calloc(5);

    int n_features = pFix->X->n;
    int k = 0;
    ffm_coef *coef =  alloc_fm_coef(n_features, k, false);
    ffm_param param = {.n_iter=500, .init_sigma=0.1, .SOLVER=SOLVER_ALS,
    .TASK=TASK_REGRESSION};
    sparse_fit(coef, pFix->X, NULL, pFix->y, NULL, param);
    sparse_predict(coef, pFix->X, y_pred);

    /* reference values from sklearn LinearRegression
    y_pred:  [ 321.05084746  346.6779661   -40.15254237  321.05084746  790.37288136]
...
void test_sparse_als_second_interactions(TestFixture_T* pFix, gconstpointer pg){
    ffm_vector * y_pred = ffm_vector_calloc(5);
...
void test_sparse_mcmc_second_interactions(TestFixture_T* pFix, gconstpointer pg){
    ...
    ffm_param param = {.n_iter=100, .init_sigma=0.1, .SOLVER=SOLVER_MCMC,
                    .TASK=TASK_REGRESSION, .rng_seed=1234};
...
void test_sparse_mcmc_second_interactions_classification(TestFixture_T* pFix,
    ...
    ffm_coef *coef =  alloc_fm_coef(n_features, k, false);
    ffm_vector * y_pred = ffm_vector_calloc(n_samples);
    ffm_param param = {.n_iter=10, .init_sigma=0.1, .SOLVER=SOLVER_MCMC,
...
void test_train_test_of_different_size(TestFixture_T* pFix,
        gconstpointer pg){
...
void test_hyerparameter_sampling(void){
    ...
    int n_features = 20; 
    int n_samples = 150;
    int k = 1; // don't just change k, the rank is hard coded in the test (ffm_vector_get(coef->lambda_V, 0);)
    ...
    double chi_alpha = 0;
    for (int i=0; i< n_replication; i++) chi_alpha += ffm_pow_2(
            gsl_cdf_ugaussian_Qinv(ffm_vector_get(alpha_rep, i)));
    g_assert_cmpfloat(gsl_ran_chisq_pdf(chi_alpha, n_replication) , < , .05);

    double chi_lambda_w = 0;
    for (int i=0; i< n_replication; i++) chi_lambda_w += ffm_pow_2(
            gsl_cdf_ugaussian_Qinv(ffm_vector_get(lambda_w_rep, i)));
    g_assert_cmpfloat(gsl_ran_chisq_pdf(chi_lambda_w, n_replication) , < , .05);

    double chi_lambda_V = 0;
    for (int i=0; i< n_replication; i++) chi_lambda_V += ffm_pow_2(
            gsl_cdf_ugaussian_Qinv(ffm_vector_get(lambda_V_rep, i)));
    g_assert_cmpfloat(gsl_ran_chisq_pdf(chi_lambda_V, n_replication) , < , .05);

    double chi_mu_w = 0;
    for (int i=0; i< n_replication; i++) chi_mu_w += ffm_pow_2(
            gsl_cdf_ugaussian_Qinv(ffm_vector_get(mu_w_rep, i)));
    g_assert_cmpfloat(gsl_ran_chisq_pdf(chi_mu_w, n_replication) , < , .05);

    double chi_mu_V = 0;
    for (int i=0; i< n_replication; i++) chi_mu_V += ffm_pow_2(
            gsl_cdf_ugaussian_Qinv(ffm_vector_get(mu_V_rep, i)));
    g_assert_cmpfloat(gsl_ran_chisq_pdf(chi_mu_V, n_replication) , < , .05);

    ffm_vector_free_all(alpha_rep, lambda_w_rep, lambda_V_rep,
                    mu_w_rep, mu_V_rep, err);
    ffm_rng_free(rng);
...
void test_train_test_data(void){
    ...
    TestFixture_T *data_train = makeTestFixture(124, n_samples_train, n_features, k);
    TestFixture_T *data_test = makeTestFixture(124, n_samples_test, n_features, k);
    ...
    ffm_param param_als = {.n_iter=200, .init_sigma=0.1, .SOLVER=SOLVER_ALS};
    sparse_fit(coef, data_train->X, data_test->X, data_train->y, y_pred, param_als);
...
void test_sparse_map_gibbs_second_interactions(void){
    ...
    ffm_coef *coef =  alloc_fm_coef(n_features, k, false);
    ffm_param param = {.n_iter=5, .init_sigma=init_sigma, .SOLVER=SOLVER_MCMC};
    sparse_fit(coef, data->X, data->X, data->y, y_pred, param);
    ...
    ffm_param param_50 = {.n_iter=50, .init_sigma=init_sigma, .SOLVER=SOLVER_MCMC};
...
void test_sparse_als_classification(void){
    ...
    ffm_coef *coef =  alloc_fm_coef(n_features, k, false);
    ffm_param param = {.n_iter=50, .init_sigma=init_sigma,
        .SOLVER=SOLVER_ALS, .TASK=TASK_CLASSIFICATION};
...
void test_sparse_als_classification_path(void){
    ...
    ffm_coef *coef =  alloc_fm_coef(n_features, k, false);
    ffm_param param = {.n_iter=0, .init_sigma=init_sigma,
        .SOLVER=SOLVER_ALS, .TASK=TASK_CLASSIFICATION};
    param.init_lambda_w = 5.5;
    param.init_lambda_V = 5.5;
...
void test_sparse_mcmc_classification(void){
    ...
    ffm_coef *coef =  alloc_fm_coef(n_features, k, false);
    ffm_param param = {.n_iter=50, .init_sigma=init_sigma,
        .SOLVER=SOLVER_MCMC, .TASK=TASK_CLASSIFICATION};
    param.init_lambda_w = 5.5;
    param.init_lambda_V = 5.5;
    sparse_fit(coef, data->X, data->X, data->y, y_pred, param);
    sparse_predict(coef, data->X, y_pred);
...
void test_numerical_stability(void){
    ...
    coef =  alloc_fm_coef(n_features, k, false);
    ffm_param param_mcmc = {.n_iter=50, .init_sigma=0.01, .SOLVER=SOLVER_MCMC};
...
void test_als_warm_start(TestFixture_T* pFix, gconstpointer pg){
    ...
    ffm_param param = {.warm_start=false, .init_sigma=0.1,
        .SOLVER=SOLVER_ALS, .TASK=TASK_REGRESSION, .rng_seed=123};
...
void test_mcmc_warm_start(TestFixture_T* pFix, gconstpointer pg){
    ...
    ffm_param param = {.warm_start=false, .init_sigma=0.1,
        .SOLVER=SOLVER_MCMC, .TASK=TASK_REGRESSION, .rng_seed=125};
...
int main(int argc, char** argv)
{
  ...
  TestFixture_T Fixture;
    ...
    g_test_add("/als/update second-order error",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_update_second_order_error,
      TestFixtureDestructor
      );

    g_test_add("/als/eval second-order term",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_eval_second_order_term,
      TestFixtureDestructor
      );

    g_test_add("/als/update v_ij",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_sparse_update_v_ij,
      TestFixtureDestructor
      );

    g_test_add("/general/predict",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_sparse_predict,
    ...
    g_test_add("/general/row_predict",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_row_predict,
    ...
    g_test_add("/als/zero order only",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorSimple,
      test_sparse_als_zero_order_only,
      TestFixtureDestructor
      );

    g_test_add("/als/first order only",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorSimple,
      test_sparse_als_first_order_only,
      TestFixtureDestructor
      );

    g_test_add("/als/second order only",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorSimple,
      test_sparse_als_second_order_only,
      TestFixtureDestructor
      );

    g_test_add("/als/all interactions",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorSimple,
      test_sparse_als_all_interactions,
      TestFixtureDestructor
      );

    g_test_add("/als/first order",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_sparse_als_first_order_interactions,
      TestFixtureDestructor
      );

    g_test_add("/als/second order",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_sparse_als_second_interactions,
      TestFixtureDestructor
      );

    g_test_add("/mcmc/second order",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
      test_sparse_mcmc_second_interactions,
      TestFixtureDestructor
      );

    g_test_add("/mcmc/second order classification",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
    ...
    g_test_add("/general/train test different size",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorLong,
    ...
    g_test_add_func("/als/generated data",
            test_sparse_als_generated_data);

    g_test_add_func("/mcmc/MAP gibbs first order",
            test_sparse_map_gibbs_first_order_interactions);

    g_test_add_func("/mcmc/hyperparameter sampling",
            test_hyerparameter_sampling);
    ...
    g_test_add_func("/als/classification path", test_sparse_als_classification_path);
    g_test_add_func("/mcmc/classification", test_sparse_mcmc_classification);

    g_test_add("/als/warm_start",
      TestFixture_T,
      &Fixture,
      TestFixtureContructorSimple,
      test_als_warm_start,
