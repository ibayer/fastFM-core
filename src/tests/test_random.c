...
void test_rng_seed(void){
    ...
    g_assert_cmpfloat(ffm_rand_normal(rng1, 0, 1), ==, ffm_rand_normal(rng2, 0, 1));
    g_assert_cmpfloat(ffm_rand_normal(rng1, 0, 1), !=, ffm_rand_normal(rng3, 0, 1));
}
...
void test_uniform_mean(void){
    ...
    for (int i=0; i<x->size; i++)
        ffm_vector_set(x, i, ffm_rand_uniform(kr));
...
void test_left_trunc_normal_mean(void){
    ...
    for (int i=0; i<x->size; i++)
        ffm_vector_set(x, i,
                trunc_mean + ffm_rand_left_trunc_normal(kr, trunc) * trunc_sigma);
    double mean = ffm_vector_mean(x);
...
int main(int argc, char** argv)
{
    ...
    g_test_add_func("/random/gamma/ mean-test (scale < 1)", test_gamma_mean_small_scale);
    ...
    g_test_add_func("/random/left trunc normal/ mean-test", test_left_trunc_normal_mean);
    g_test_add_func("/random/left trunc normal/ mean-test (neg trunc)",
            test_left_trunc_normal_mean_neg_trunc);
    g_test_add_func("/random/left trunc normal/ var-test", test_left_trunc_normal_var);
