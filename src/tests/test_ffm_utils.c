...
void test_ffm_normal_pdf(void){
    double x[] = {0.1, 1.2, 2.3, -1.1, -3.3, -7.7};
    double phi[] = {3.96952547477011808e-01,
                    1.94186054983212952e-01,
                    2.83270377416011861e-02,
                    2.17852177032550526e-01,
                    1.72256893905368123e-03,
                    5.32414837225295172e-14};
    for (int i=0; i<6; i++)
...
void test_ffm_vector_sort(void){
    ...
    ffm_vector_sort(&v);
    for (int i=0; i<5; i++)
       assert(ffm_vector_get(&v, i) == values_sorted[i]);
}
...
void test_Cs_write(void){
    ...
    cs *X = cs_spalloc (m, n, m*n, 1, 1) ;    /* create triplet identity matrix */
    cs_entry(X, 0, 0, 6); cs_entry(X, 0, 1, 1);
    cs_entry(X, 1, 0, 2.22); cs_entry(X, 1, 1, 3);
    cs_entry(X, 2, 0, 3.333);
    cs_entry(X, 3, 0, 6); cs_entry(X, 3, 1, 1);
    cs_entry(X, 4, 0, 4.4444); cs_entry(X, 4, 1, 5);
    //printf ("X:\n") ; cs_print (X, 0) ; /* print A */
...
void test_ffm_vector_functions(void){
    ...
    for (int i=0; i<size; i++)
        g_assert_cmpfloat(a->data[i], ==, b->data[i]);
...
void test_ffm_matrix_functions(void){
    ...
    for (int i=0; i<X->size0 * X->size1; i++)
            X->data[i] = i;
    ...
    for (int i=0; i<Y->size0; i++)
        for (int j=0; j<Y->size1; j++)
            g_assert_cmpfloat(ffm_matrix_get(Y, i, j), ==,
                    ffm_matrix_get(X, i, j));
...
int main(int argc, char** argv)
{
    ...
    g_test_add_func("/utils/ffm_vector/ make labels", test_ffm_vector_make_labels);
    ...
    g_test_add_func("/utils/ffm_vector/ operations",
            test_ffm_vector_functions);
    ...
    g_test_add_func("/utils/read ffm_matrix_from_file", test_read_ffm_matrix_from_file);
    g_test_add_func("/utils/read svm_light file", test_read_svm_light_file);
    g_test_add_func("/utils/read svm_light file w.o. target", test_read_svm_light_file_without_target);
