...
double ffm_normal_pdf(double x){
    return exp(-(x*x) / 2.0) / sqrt(M_PI * 2);
}
...
double ffm_vector_accuracy(ffm_vector *y_true, ffm_vector *y_pred){
    ...
    for (int i=0; i<y_true->size; i++){
        if (ffm_vector_get(y_true, i) >= .0 &&
                ffm_vector_get(y_pred, i) >= 0.5)
            acc++;
        else if (ffm_vector_get(y_true, i) < .0 &&
                ffm_vector_get(y_pred, i) < 0.5)
            acc++;
    }
    if (acc == 0) return 0;
...
double ffm_vector_median(ffm_vector *y){
    ...
    if ( y->size % 2 == 0)
        median = (ffm_vector_get(cp, y->size / 2) + \
                    ffm_vector_get(cp, (y->size / 2) -1)) / 2.0;
    else median = ffm_vector_get(cp, y->size / 2);
    ffm_vector_free(cp);
    return median;
}
...
double ffm_r2_score(ffm_vector *y_true, ffm_vector *y_pred){
    ...
    ss_tot *=  n_samples;
    double ss_res = 0;
    for (int i=0; i<y_pred->size; i++) ss_res += \
        ffm_pow_2(ffm_vector_get(y_true, i) - ffm_vector_get(y_pred, i));
...
int Cs_daxpy (const cs *A, int col_index, double alpha, const double *x, double *y)
{
    int p, j, *Ap, *Ai ;
    double *Ax ;
